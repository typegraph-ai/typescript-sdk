# TypeGraph Self-Hosted Setup

Full control over your data, embedding models, and infrastructure. You bring the database and embedding provider; TypeGraph handles the rest.
...
npm install @typegraph-ai/adapter-pgvector      # Production - Postgres + pgvector
# npm install @typegraph-ai/adapter-sqlite-vec  # Local dev - zero external dependencies
# ... or any other pre-built, or custom vector store adapters

# Pick integrations (optional - 3rd party connectors)
#npm install @typegraph-ai/integration-core          # Shared integration types
#npm install @typegraph-ai/integration-slack         # Slack messages & channels
#npm install @typegraph-ai/integration-google-drive  # Google Drive files
#npm install @typegraph-ai/integration-hubspot       # HubSpot CRM contacts, companies, deals
...
```ts
// TypeGraph specific import stuff
...
When you call `typegraph.deploy()`, TypeGraph creates these tables in your database:

| Table | Purpose |
|-------|---------|
| `typegraph_documents` | Document records with identity columns (tenant_id, group_id, user_id, agent_id, conversation_id) and visibility |
| `typegraph_hashes` | Content hashes for deduplication and incremental sync |
| `typegraph_buckets` | Bucket registry with identity columns |
| `typegraph_jobs` | Scheduled job definitions with identity columns |
| `typegraph_chunks_registry` | Registry of which embedding models have been used |
...
```sql
typegraph_chunks_openai_text_embedding_3_small (
  id              UUID PRIMARY KEY,
  bucket_id       TEXT,
  tenant_id       TEXT,
  group_id        TEXT,
  ...
  document_id     UUID REFERENCES typegraph_documents,
  content         TEXT,
...
Each embedding model gets its own table. If you later switch models or use different models per source, TypeGraph isolates them automatically.

## 4) Ingest Documents

```ts
// Send documents to your FAQ bucket - TypeGraph handles chunking and embedding under the hood
...
3. Checks `typegraph_hashes` -- skips if content unchanged (idempotent re-ingestion)
4. Inserts a record into `typegraph_documents` (title, url, content_hash, status, ...)
...
```sql
INSERT INTO typegraph_chunks_openai_text_embedding_3_small
...
8. Updates `typegraph_hashes` so the next `ingest()` call can skip unchanged docs
...
TypeGraph runs a hybrid search combining vector similarity and full-text keyword matching:

1. **Groups sources by embedding model.** If you have sources using different models (e.g., OpenAI `text-embedding-3-small` for docs, Cohere `embed-v4` for support tickets), TypeGraph handles each model separately -- embedding your query with each model, searching each model's table, then merging all results together with RRF re-ranking. You just call `query()` once and get back a single ranked result set. The multi-model complexity is invisible to you.
...
WITH vector_ranked AS (
  ...
  FROM typegraph_chunks_openai_text_embedding_3_small
  WHERE bucket_id = 'faq'
  LIMIT 60
),
keyword_ranked AS (
...
4. **Maps results** back to TypeGraph result objects with scores, source info, and chunk positions.
...
const md = typegraph.assemble(response.results, { format: 'markdown' })
// # How do I set up SSO?
// To enable SSO, navigate to Settings > Authentication...
//
// ---
...
const wiki = await typegraph.buckets.create({ name: 'wiki' })
```

### What Happens at Query Time

When you call `typegraph.query()`, TypeGraph:
...
You don't think about which model applies to which bucket -- TypeGraph handles the fan-out and merge.

### Per-Model Table Isolation

Each embedding model gets its own vector table (e.g., `typegraph_chunks_openai_text_embedding_3_small`, `typegraph_chunks_cohere_embed_english_v3_0`). This means:
...
await typegraph.ingest(bucket.id, documents, {
...
const { results } = await typegraph.query('SSO setup', {
  tenantId: 'acme-corp',
  userId: 'alice',
})
```
...
const adapter = new PgVectorAdapter({
  ...
  schema: 'cust_abc',  // tables created as cust_abc.typegraph_documents, etc.
})
```
...
For zero-infrastructure development, see the [TypeGraph Cloud Quickstart](../TypeGraph%20Cloud/quickstart.md). For local development without external dependencies, see the [Local Dev Guide](../Local%20Dev/getting-started.md).
