#!/usr/bin/env npx tsx
/**
 * Contractual Clause Retrieval — typegraph Graph (Neural Search)
 *
 * Runs the contractual-clause-retrieval benchmark
 * using typegraph graph with neural search (hybrid + memory recall + PPR graph traversal).
 *
 * Usage:
 *   npx tsx --env-file=.env contractual-clause-retrieval/neural/run.ts              # query-only
 *   npx tsx --env-file=.env contractual-clause-retrieval/neural/run.ts --seed        # re-index
 *   npx tsx --env-file=.env contractual-clause-retrieval/neural/run.ts --validate    # smoke test
 *   npx tsx --env-file=.env contractual-clause-retrieval/neural/run.ts --record      # save to history
 */
...
const config = getConfig('contractual-clause-retrieval/neural')
