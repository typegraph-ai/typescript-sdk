#!/usr/bin/env npx tsx
/**
 * License TLDR Retrieval — typegraph Graph (Neural Search)
 *
 * Runs the license-tldr-retrieval benchmark
 * using typegraph graph with neural search (hybrid + memory recall + PPR graph traversal).
 *
 * Usage:
 *   npx tsx --env-file=.env license-tldr-retrieval/neural/run.ts              # query-only
 *   npx tsx --env-file=.env license-tldr-retrieval/neural/run.ts --seed        # re-index
 *   npx tsx --env-file=.env license-tldr-retrieval/neural/run.ts --validate    # smoke test
 *   npx tsx --env-file=.env license-tldr-retrieval/neural/run.ts --record      # save to history
 */
...
const config = getConfig('license-tldr-retrieval/neural')
