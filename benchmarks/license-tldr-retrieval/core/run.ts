...
async function main() {
  ...
  if (!bucket || shouldSeed) {
    ...
    for (const doc of corpus) {
      ...
      ingested++
      if (ingested % 50 === 0 || ingested === corpus.length) {
  ...
  printResults(result)
  writeFileSync('./license-results-core.json', JSON.stringify(result, null, 2))
  writeFileSync('./license-results-core.md', formatMarkdown(result))
  console.log('  Results: license-results-core.json, license-results-core.md')
  console.log('══════════════════════════════════════════════════════')
}
