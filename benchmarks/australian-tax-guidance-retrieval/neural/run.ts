#!/usr/bin/env npx tsx
/**
 * AU Tax Guidance Retrieval — typegraph Graph (Neural Search)
 *
 * Runs the australian-tax-guidance-retrieval benchmark
 * using typegraph graph with neural search (hybrid + memory recall + PPR graph traversal).
 *
 * Usage:
 *   npx tsx --env-file=.env australian-tax-guidance-retrieval/neural/run.ts              # query-only
 *   npx tsx --env-file=.env australian-tax-guidance-retrieval/neural/run.ts --seed        # re-index
 *   npx tsx --env-file=.env australian-tax-guidance-retrieval/neural/run.ts --validate    # smoke test
...
const config = getConfig('australian-tax-guidance-retrieval/neural')
