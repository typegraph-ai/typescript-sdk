#!/usr/bin/env npx tsx
/**
 * MLEB-SCALR — typegraph Graph (Neural Search)
 *
 * Runs the mleb-scalr benchmark
 * using typegraph graph with neural search (hybrid + memory recall + PPR graph traversal).
 *
 * Usage:
 *   npx tsx --env-file=.env mleb-scalr/neural/run.ts              # query-only
 *   npx tsx --env-file=.env mleb-scalr/neural/run.ts --seed        # re-index
 *   npx tsx --env-file=.env mleb-scalr/neural/run.ts --validate    # smoke test
 *   npx tsx --env-file=.env mleb-scalr/neural/run.ts --record      # save to history
 */
...
const config = getConfig('mleb-scalr/neural')
