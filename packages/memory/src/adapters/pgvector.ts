...
export class PgMemoryStoreAdapter implements MemoryStoreAdapter {
  ...
  async findEntities(query: string, scope: MemoryScope, limit?: number): Promise<SemanticEntity[]> {
    const rows = await this.sql(
  ...
  async searchEntities(embedding: number[], scope: MemoryScope, limit?: number): Promise<SemanticEntity[]> {
    const vectorStr = `[${embedding.join(',')}]`
    const rows = await this.sql(
...
function mapRowToMemory(row: Record<string, unknown>): MemoryRecord {
  const base: MemoryRecord = {
    ...
    scope: parseJson(row.scope) as MemoryScope,
